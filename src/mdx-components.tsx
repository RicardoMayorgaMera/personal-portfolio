...
function turnChildrenToString(children: ReactNode) {
...
function getSlug(children: ReactNode) {
  return turnChildrenToString(children)
    .toLowerCase()
...
const textBaseCn = "max-w-216 mx-auto relative my-0";

const hBaseCn = `${textBaseCn} mt-8 font-semibold tracking-tight scroll-mt-24`; // Base className headings

const customComponents: MDXComponents = {
  p: (props) => (
    <p className={`${textBaseCn} mt-4 text-base leading-normal`} {...props} />
  ),
  strong: (props) => (
    <strong className="text-teal-600 font-semibold" {...props} />
  ),
  h1: ({ children, ...props }) => (
    <a
      href={`#${getSlug(children)}`}
      id={getSlug(children)}
  ...
  h2: ({ children, ...props }) => (
    ...
    >
      <h2
        className={`${hBaseCn} text-3xl text-purple-500 font-bold leading-snug`}
        {...props}
      >
        <span className="text-gray-50 -left-6 opacity-0 group-hover:opacity-100 absolute no-underline -translate-x-full duration-200 transition-opacity">
          #
  ...
  h3: ({ children, ...props }) => (
    ...
    >
      <h3
        className={`${hBaseCn} text-2xl text-red-500 font-medium leading-snug"`}
        {...props}
      >
        <span className="text-gray-50 -left-6 opacity-0 group-hover:opacity-100 absolute no-underline -translate-x-full duration-200 transition-opacity">
          #
  ...
  h4: ({ children, ...props }) => (
    ...
    >
      <h4
        className={`${hBaseCn} text-xl text-teal-500 font-semibold"`}
        {...props}
      >
        <span className="text-gray-50 -left-6 opacity-0 group-hover:opacity-100 absolute no-underline -translate-x-full duration-200 transition-opacity">
          #
  ...
  ul: (props) => (
    <ul className="list-disc list-inside mt-4 space-y-1" {...props} />
  ),
  ol: (props) => (
    <ol className="list-decimal list-inside mt-4 space-y-1" {...props} />
  ),
  li: (props) => <li className="ml-6 leading-relaxed" {...props} />,
  blockquote: (props) => (
    <blockquote
      className="relative mt-4 border-l-4 border-orange-300 bg-orange-50 text-gray-700 p-4 rounded-r-lg max-w-3xl mx-auto"
      {...props}
    />
  ),
  code: (props) => (
  ...
  pre: ({ className, ...props }) => {
    ...
    return (
      <div className={`${textBaseCn} my-4 rounded-md group `}>
        <Code
          lang={lang}
          theme={codeTheme}
        ...
        <div className="absolute right-5 top-1/2 -translate-y-1/2">
          <CopyButton
            rawContent={turnChildrenToString(props.children.props.children)}
          />
        </div>
      </div>
    );
  ...
  a: ({ href, ...props }) => {
    if (href?.startsWith("/")) {
      return (
        <Link
          href={href}
          className="text-blue-500 underline decoration-2 underline-offset-4 hover:text-blue-600 transition-colors"
          {...props}
        />
      );
    }
